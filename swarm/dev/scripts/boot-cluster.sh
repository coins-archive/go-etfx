...
#
# The cluster contains a bootnode, a getfx node and multiple swarm nodes, with
...
# static getfx configuration
GETFX_IP="192.168.33.3"
GETFX_RPC_PORT="8545"
GETFX_RPC_URL="http://${GETFX_IP}:${GETFX_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the getfx and
# swarm nodes
start_bootnode() {
...
# start_getfx_node starts a getfx node with --datadir pointing at <base-dir>/getfx
# and a single, unlocked account with password "getfx"
start_getfx_node() {
  local dir="${base_dir}/getfx"
  mkdir -p "${dir}"

  local password="getfx"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating getfx account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get getfx account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GETFX_IP}"
    --rpcport   "${GETFX_RPC_PORT}"
    --verbosity "6"
  )

  start_node "getfx" "${GETFX_IP}" "$(which getfx)" ${args[@]}
}
...
create_account() {
  ...
  getfx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
